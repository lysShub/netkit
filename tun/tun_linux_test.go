...
func Test_Tap(t *testing.T) {
	addr := netip.PrefixFrom(LocIP().Next(), 32)
...
func Test_Tap_Conn_Server(t *testing.T) {
	...
	conn, err := net.DialTCP("tcp",
		&net.TCPAddr{IP: addr.Addr().AsSlice(), Port: int(RandPort())},
		&net.TCPAddr{IP: rip, Port: 80},
	)
	require.NoError(t, err)
	defer conn.Close()
...
func Test_Tap_Loopback(t *testing.T) {
	var (
		...
		// caddr = netip.AddrPortFrom(LocIP().Next(), 19986)
		saddr = netip.AddrPortFrom(LocIP(), 8080)
	)
	...
	conn, err := net.DialTCP("tcp",
		...
		&net.TCPAddr{IP: LocIP().AsSlice(), Port: 8080},
	)
	require.NoError(t, err)
